...
public class ExcelFilesFirmas {
	


	public static byte[] generarExcelFirmas(List<FilesFirmaAbogadoObj> listaFirmas,String firmaAbogado) throws IOException {
		boolean local=false;
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            Sheet sheet = workbook.createSheet("Files Firm "+firmaAbogado);
            ...
            // Cabecera
            String[] headers = {"File", "Email", "Cliente", "Lawyer", "Lawyer email","State"};
            ...
            // Contenido
            int rowIdx = 1;
            for (FilesFirmaAbogadoObj f : listaFirmas) {
            	
                Row row = sheet.createRow(rowIdx++);
                Cell cell0 = row.createCell(0);
                cell0.setCellValue(f.getIdSolicitud());
                cell0.setCellStyle(csn);

                Cell cell1 = row.createCell(1);
                cell1.setCellValue(f.getEmail());
                cell1.setCellStyle(csn);

                Cell cell2 = row.createCell(2);
                cell2.setCellValue(f.getCliente());
                cell2.setCellStyle(csn);

                Cell cell3 = row.createCell(3);
                cell3.setCellValue(f.getAbogado());
                cell3.setCellStyle(csn);

                Cell cell4 = row.createCell(4);
                cell4.setCellValue(f.getEmailAbogado());
                cell4.setCellStyle(csn);
                
                Cell cell5 = row.createCell(5);
                cell5.setCellValue(f.getEstado());
