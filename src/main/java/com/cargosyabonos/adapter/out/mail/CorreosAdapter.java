...
public class CorreosAdapter implements CorreosPort {

	private static final Logger logger = LoggerFactory.getLogger(CorreosAdapter.class);
	...
	public void enviarCorreoDeLayoutCalendarWithInvite(String tipoInvitacion,String email, String subject, Map<String, Object> params, String layout,String uidSchedule) {
		...
		ConfiguracionEntity conf = confPort.obtenerConfiguracionPorCodigo("CONTACT-INFO");

		params.put("${rutahost}", rutaServer());
		params.put("${empresa}", conf.getValor());
		UtilidadesAdapter.pintarLog("ruta server:"+rutaServer());
		params.put("${url}", rutaServer());
		...
		try {
			...
			envioCorreo.sendMailWithSchedule(tipoInvitacion,email, subject, template, evento,uidSchedule);
